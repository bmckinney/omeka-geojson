            ...
            that._geojson.features.forEach(function(marker) {
                ...
                if( marker.geometry.type==='Point'){ // TODO: Add support for lines and polygons
                    ...
                    var popupHTML = (typeof marker.properties.title !== 'undefined'?`<h3>${marker.properties.title}</h3>` : '' ) +
                        (typeof marker.properties.image !== 'undefined'? `<a href='${marker.properties.website}' target='_blank'><img src='${marker.properties.image}' width=200 alt='${marker.properties.title}'></a>` : '' ) +
                        (typeof marker.properties.description !== 'undefined'? `<p>${marker.properties.description}</p>` : `<p>${that.options.popup.description}</p>` ) +
                        (typeof marker.properties.website !== 'undefined'? `<a href='${marker.properties.website}' target='_blank' class='button'>See more</a>` : `<a href='https://www.openstreetmap.org/?mlat=${marker.geometry.coordinates[1]}&mlon=${marker.geometry.coordinates[0]}' target='_blank' class='button'>Browse Location</a>`);
